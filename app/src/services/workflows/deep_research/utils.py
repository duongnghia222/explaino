...
from src.services.workflows.deep_research.configuration import Configuration, SearchAPI
from src.services.workflows.deep_research.prompts import summarize_webpage_prompt
from src.services.workflows.deep_research.state import ResearchComplete, Summary
...
async def tavily_search(
    ...
    if model_base_url:
        init_kwargs["base_url"] = model_base_url
...
def get_base_url_for_model(model_name: str, config: RunnableConfig):
    """Get custom base URL for a model (e.g. for OpenRouter)."""
    configurable = Configuration.from_runnable_config(config)
    model_name = model_name.lower()
    if model_name.startswith("openai:") and configurable.openai_base_url:
        return configurable.openai_base_url
